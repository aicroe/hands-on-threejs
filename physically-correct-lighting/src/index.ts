...
let params: {
  ...
  luminanceOpts: Record<string, number>,
  ambientOpts: Record<string, number>,
  toneMappingOpts: Record<string, THREE.ToneMapping>,
}
let lights: {
  ambient: THREE.HemisphereLight,
  spot: THREE.SpotLight,
  spotHelper: THREE.SpotLightHelper,
  spotCameraHelper: THREE.CameraHelper,
};
...
function init() {
  ...
  // renderer.gammaInput = true; https://learnopengl.com/Advanced-Lighting/Gamma-Correction
  ...
  /**
   * The shaders associated with MeshStandardMaterial and MeshPhysicalMaterial use the
   * light's power, when renderer.physicallyCorrectLights is true.
   *
   * 1700 lm is about 100-watt tungsten bulb
   * A tungsten bulb has a rating of about 17 lumens per watt.
   * - 17 lumens/watt
   * A LED bulb has a rating of more than 100 lumens per watt.
   * - >100 lumens/watt
   * https://www.rapidtables.com/calc/light/how-lumen-to-watt.html
   */
  lights.spot.power = 1700; // Sets the light's luminance, measured in lumens?
  ...
  gui.add(params, 'exposure', 0, 1).onChange((value) => {
    /**
     * Using the power of 5, to allow for very bright scenes.
     * And also to have a logarithmic effect when sliding the exposure value.
     */
    renderer.toneMappingExposure = Math.pow(value, 5.0);
  });
